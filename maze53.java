...
Â * the desired number of rows and columns.
 * 
...
public class maze53 {
    ...
    public static void main(String[] args) {
        int width  = 810;    //693 760
        int height = 545;    //545
        mazeFrame = new JFrame("Maze 5.3");
...
Â Â Â Â Â  * and contains all the functionality of the program.
      */
...
Â Â Â Â Â Â Â Â Â * according their 'f' field
         */
        ...
        /**
         * Auxiliary class that specifies that the cells will be sorted
Â Â Â Â Â Â Â Â Â * according their 'dist' field
         */
...
Â Â Â Â Â Â Â Â Â * obstacles or move the robot and/or target.
         */
        ...
        // basic buttons
        JButton SButton,LButton,resetButton, mazeButton, clearButton, realTimeButton, stepButton, animationButton;
        ...
        public MazePanel(int width, int height) {
            ...
            SButton.addActionListener(new ActionHandler());
            SButton.setBackground(Color.lightGray);
            SButton.setToolTipText
                    ("Saves maze");
            SButton.addActionListener(this::SButtonActionPerformed);
            
//            ///////////////////////////////////////
            LButton = new JButton("LOAD");
            LButton.addActionListener(new ActionHandler());
            ...
            ///////////////////////////////////////////////////////////////////
            SButton.setBounds(700,65,100,25);
            LButton.setBounds(700,95,100,25);
            //////////////////////////////////////////////////////////////////
    ...
    static protected JSpinner addLabeledSpinner(Container c,
        ...
        private void SButtonActionPerformed(java.awt.event.ActionEvent evt) {                                           
            //AGENT POSITION NOT INCLUDED//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            
        
            JFileChooser chooser = new JFileChooser(".");
                
            int response1;
            response1 =chooser.showSaveDialog(SButton);
            ...
            try {
                ...
                int Rx = robotStart.row;
                int Ry = robotStart.col;
                int Tx = targetPos.row;
                int Ty = targetPos.col;
                ...
                if (searching){
                	int Cx = openSet.get(0).row;
                    int Cy = openSet.get(0).col;
                    
                    for (int x = 0; x < rows; x++) {
                        String str2 ="";
                        for (int y = 0; y < columns; y++){

                            String str1 = Integer.toString(grid[x][y]);
        ...
        private void LButtonActionPerformed(java.awt.event.ActionEvent evt) {                                           
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    Scanner myReader2 = new Scanner(selectedFile);
                    ...
                    for (int i = 0; i < data2.length(); i++) {
                        if(Character.compare(data2.charAt(i), '#')==0){
                            grid[Crow][i]=OBST;
                        }else if(Character.compare(data2.charAt(i), 'X')==0){
                            Cell c = new Cell(0,0);
                            c.row=Crow;
                            c.col=i;
                            closedSet.add(c);
                        ...
                        else if(Character.compare(data2.charAt(i), 'S')==0 ){
                            robotStart = new Cell(Crow,i);
                            openSet.add(robotStart);
                            grid[robotStart.row][robotStart.col] = ROBOT;
                            
                        }
                        else if (Character.compare(data2.charAt(i), 'A')==0){
                            robotStart = new Cell(Crow,i);
                            openSet.add(0,robotStart);
                            grid[robotStart.row][robotStart.col] = FRONTIER;
                        }
                        else if (Character.compare(data2.charAt(i), 'O')==0){
                            //robotStart = new Cell(Crow,i);
                            Cell o = new Cell(0,0);
                            o.row=Crow;
                            o.col=i;
                            openSet.add(o);
                            grid[o.row][o.col] = FRONTIER;
                        }
                        else if(Character.compare(data2.charAt(i), 'G')==0){
                            targetPos = new Cell(Crow,i);
                            //openSet.add(robotStart);
         ...
         */
        public void initializeGrid(Boolean makeMaze) {                                           
        ...
        /**
Â Â Â Â Â Â Â Â  * Calculates the path from the target to the initial position
Â Â Â Â Â Â Â Â  * of the robot, counts the corresponding steps
Â Â Â Â Â Â Â Â  * and measures the distance traveled.
Â Â Â Â Â Â Â Â  */
        ...
        /**
 Â Â Â Â Â Â Â  * Gives initial values â€‹â€‹for the cells in the grid.
Â Â Â Â Â Â Â Â  * With the first click on button 'Clear' clears the data
Â Â Â Â Â Â Â Â  * of any search was performed (Frontier, Closed Set, Route)
Â Â Â Â Â Â Â Â  * and leaves intact the obstacles and the robot and target positions
Â Â Â Â Â Â Â Â  * in order to be able to run another algorithm
Â Â Â Â Â Â Â Â  * with the same data.
Â Â Â Â Â Â Â Â  * With the second click removes any obstacles also.
Â Â Â Â Â Â Â Â  */
        ...
        /**
Â Â Â Â Â Â Â Â Â  * Appends to the list containing the nodes of the graph only
Â Â Â Â Â Â Â Â Â  * the cells belonging to the same connected component with node v.
Â Â Â Â Â Â Â Â Â  * This is a Breadth First Search of the graph starting from node v.
Â Â Â Â Â Â Â Â Â  *
Â Â Â Â Â Â Â Â Â  * @param v    the starting node
Â Â Â Â Â Â Â Â Â  */
         ...
         *Â 
         *Â When one thinks of Wikipedia pseudocode, observe that the
         * algorithm is still looking for his target while there are still
         * nodes in the queue Q.
         *Â Only when we run out of queue and the target has not been found,
         * can answer that there is no solution .
         *Â As is known, the algorithm models the problem as a connected graph.
         ...
         *Â To be thus possible negative response from the algorithm,
